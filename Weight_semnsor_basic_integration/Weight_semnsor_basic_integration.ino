...
void sensorTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (scale.is_ready()) {
      weightKgGlobal = scale.get_units(); // Get weight in calibrated units (e.g., kg)
      Serial.print("Weight: ");
      Serial.print(weightKgGlobal);
      Serial.println(" kg");
    } else {
...
void uploadTask(void *pvParameters) {
  ...
  for (;;) {
    if (WiFi.status() == WL_CONNECTED && !isnan(temperature) && !isnan(humidity) && waterLevelPercentage != -1) {
      ...
      ThingSpeak.setField(6, weightKgGlobal); // Use the global weight variable
